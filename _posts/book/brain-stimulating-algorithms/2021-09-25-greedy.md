...
[최소 신장 트리](파일명쓰기)는 다음과 같은 그래프가 있다고 했을 때, 그래프 내의 정점을 최소의 비용으로 연결하는 트리를 말한다.<br>
...
[다익스트라의 최단 경로 알고리즘](링크걸기)은 그래프 내의 한 정점에서 다른 정점으로 향하는 가장 짧은 경로를 구하는 알고리즘이다.<br>
...
PriorityQueue.h, PriorityQueue.c는 [우선순위 큐 예제 프로그램](https://garamm.github.io/파일명쓰기){:target="_blank"}를 사용하면 된다.<br>
Huffman.h
```c
#ifndef HUFFMAN_H
#define HUFFMAN_H
